...
function DsPaymentOptionsCard({
  ...
  return (
    <div className={cn("flex flex-col gap-6 rounded-2xl bg-white px-8 py-10", className)}>
      {/* Header */}
      {(title || subtitle) && (
        <div className="flex flex-col gap-2 leading-[1.3]">
          {title && <p className="text-2xl font-medium tracking-[-0.96px] text-black">{title}</p>}
          {subtitle && (
            <p className="text-base tracking-[-0.64px] text-nova-primary-dark">{subtitle}</p>
          )}
        </div>
      )}
          ...
          >
            <div className="flex h-6 items-center justify-between">
              ...
              <DsSwitch
                ...
                onCheckedChange={(checked) => onOptionToggle(option.id, checked)}
              />
            </div>
