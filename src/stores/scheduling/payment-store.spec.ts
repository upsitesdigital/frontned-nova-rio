...
vi.mock("@/stores/auth/registration-store", () => ({
  useRegistrationStore: {
    getState: () => ({ email: "" }),
  },
}));

vi.mock("@/stores/client/services-store", () => ({
  useServicesStore: {
    getState: () => ({ selectedServiceId: null }),
  },
}));

vi.mock("@/stores/scheduling/scheduling-store", () => ({
  useSchedulingStore: {
    getState: () => ({
      selectedDate: null,
      selectedTime: null,
...
vi.mock("@/stores/scheduling/address-store", () => ({
  useAddressStore: {
...
const { useSchedulingStore } = await import("@/stores/scheduling/scheduling-store");
const { useAddressStore } = await import("@/stores/scheduling/address-store");
...
describe("PaymentStore", () => {
  ...
  describe("pay", () => {
    it("should return false when validation fails", async () => {
      ...
      expect(result).toBe(false);
      expect(SubmitPayment.submitPayment).not.toHaveBeenCalled();
    });
    ...
    it("should call SubmitPayment.submitPayment and return true on success", async () => {
