...
const { validatePasswordStrength, validatePasswordMatch } =
  await import("@/validation/password-strength-schema");
...
describe("PasswordChangeStore", () => {
  ...
  describe("submitPasswordChange", () => {
    it("should advance to verify step on success", async () => {
      vi.mocked(ProfileApi.requestPasswordChange).mockResolvedValue(undefined as never);
    ...
    it("should set error on API failure", async () => {
      vi.mocked(ProfileApi.requestPasswordChange).mockRejectedValue(new Error("Network"));

      const result = await usePasswordChangeStore.getState().submitPasswordChange();

      expect(result).toBe(false);
      expect(usePasswordChangeStore.getState().error).toBe("Erro ao solicitar alteracao de senha.");
  ...
  describe("submitPasswordVerification", () => {
    it("should return false when passwords do not match", async () => {
      ...
      expect(ProfileApi.verifyPasswordChange).not.toHaveBeenCalled();
    });
    ...
    it("should verify password and close dialog on success", async () => {
      vi.mocked(ProfileApi.verifyPasswordChange).mockResolvedValue(undefined as never);
      usePasswordChangeStore.setState({
      ...
      expect(result).toBe(true);
      expect(ProfileApi.verifyPasswordChange).toHaveBeenCalledWith("123456", "Strong@1");
    ...
    it("should set error on API failure", async () => {
      vi.mocked(ProfileApi.verifyPasswordChange).mockRejectedValue(new Error("Bad code"));
      usePasswordChangeStore.setState({
        newPassword: "Strong@1",
        confirmPassword: "Strong@1",
        passwordCode: "wrong",
