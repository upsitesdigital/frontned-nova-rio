...
describe("PaymentsPageStore", () => {
  ...
  describe("loadPayments", () => {
    ...
    it("should set error on non-abort failure", async () => {
      vi.mocked(PaymentsApi.fetchClientPayments).mockRejectedValue(new Error("Network error"));
    ...
    it("should not set error on AbortError", async () => {
      ...
      vi.mocked(PaymentsApi.fetchClientPayments).mockRejectedValue(abortError);
  ...
  describe("setFilter", () => {
    it("should update filter, reset page to 1, and trigger load", async () => {
      ...
      expect(state.page).toBe(1);
      expect(PaymentsApi.fetchClientPayments).toHaveBeenCalled();
    });
  });
