...
vi.mock("@/api/client/services-api", () => ({
  ServicesApi: { fetchPublicServices: vi.fn() },
}));
...
const { ServicesApi } = await import("@/api/client/services-api");
...
describe("ServicesStore", () => {
  ...
  describe("loadServices", () => {
    it("should load services on success", async () => {
      ...
      vi.mocked(ServicesApi.fetchPublicServices).mockResolvedValue(services);
    ...
    it("should set error on failure", async () => {
      vi.mocked(ServicesApi.fetchPublicServices).mockRejectedValue(new Error("Network"));
    ...
    it("should set isLoadingServices during API call", async () => {
      ...
      vi.mocked(ServicesApi.fetchPublicServices).mockReturnValue(
        new Promise((resolve) => {
          resolveLoad = resolve;
        }),
      );
    ...
    it("should clear previous error on new load", async () => {
      ...
      vi.mocked(ServicesApi.fetchPublicServices).mockResolvedValue([]);
