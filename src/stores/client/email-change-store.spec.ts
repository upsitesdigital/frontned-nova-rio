...
vi.mock("@/stores/client/profile-info-store", () => {
...
describe("EmailChangeStore", () => {
  ...
  describe("submitEmailChange", () => {
    ...
    it("should advance to code step on success", async () => {
      ...
      vi.mocked(ProfileApi.requestEmailChange).mockResolvedValue(undefined as never);
    ...
    it("should set error on API failure", async () => {
      vi.mocked(isValidEmail).mockReturnValue(true);
      vi.mocked(ProfileApi.requestEmailChange).mockRejectedValue(new Error("Network"));
      useEmailChangeStore.setState({ newEmail: "valid@test.com" });
  ...
  describe("submitEmailVerification", () => {
    it("should verify code, reload profile, and close dialog on success", async () => {
      ...
      vi.mocked(ProfileApi.verifyEmailChange).mockResolvedValue(undefined as never);
      vi.mocked(ProfileApi.fetchClientProfile).mockResolvedValue(mockProfile as never);
      ...
      expect(result).toBe(true);
      expect(ProfileApi.verifyEmailChange).toHaveBeenCalledWith("123456", "new@test.com");
      expect(ProfileApi.fetchClientProfile).toHaveBeenCalled();
    ...
    it("should set error on API failure", async () => {
      vi.mocked(ProfileApi.verifyEmailChange).mockRejectedValue(new Error("Bad code"));
