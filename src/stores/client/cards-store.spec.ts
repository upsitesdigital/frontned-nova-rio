...
describe("CardsStore", () => {
  ...
  describe("initial state", () => {
    it("should have correct defaults", () => {
      ...
      expect(state.addForm).toEqual(emptyForm);
  ...
  describe("loadCards", () => {
    it("should load cards on success", async () => {
      vi.mocked(LoadClientCards.loadClientCards).mockResolvedValue({
    ...
    it("should set isLoading during API call", async () => {
      ...
      vi.mocked(LoadClientCards.loadClientCards).mockReturnValue(
        new Promise((resolve) => {
  ...
  describe("addCard", () => {
    it("should not proceed when validation fails", async () => {
      ...
      expect(AddClientCard.addClientCard).not.toHaveBeenCalled();
    });

    it("should add card, close dialog, and show toast on success", async () => {
      const newCard = { ...mockCard, id: 2 };
      vi.mocked(AddClientCard.addClientCard).mockResolvedValue({
        success: true,
  ...
  describe("openAddDialog / closeAddDialog", () => {
    it("should open dialog and reset form", () => {
      useCardsStore.setState({
        addForm: { ...emptyForm, cardNumber: "123" },
