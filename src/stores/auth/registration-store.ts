...
import { SubmitRegistration } from "@/use-cases/auth/submit-registration";
...
const useRegistrationStore = create<RegistrationStore>()((set, get) => ({
  ...
  submit: async () => {
    ...
    const apiErrors = await SubmitRegistration.submitRegistration({ name, email, phone, password });
