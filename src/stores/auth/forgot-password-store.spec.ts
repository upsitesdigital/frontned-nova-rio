...
const { RequestPasswordReset } = await import("@/use-cases/auth/request-password-reset");
const { ResetUserPassword } = await import("@/use-cases/auth/reset-user-password");
const { getPasswordHints, validateResetPassword } =
  await import("@/validation/reset-password-schema");
...
describe("ForgotPasswordStore", () => {
  ...
  describe("submitEmailStep", () => {
    it("should return false and set error when email is empty", async () => {
      ...
      expect(RequestPasswordReset.requestPasswordResetCode).not.toHaveBeenCalled();
    });

    it("should advance to code step on success", async () => {
      useForgotPasswordStore.setState({ email: "user@test.com" });
      vi.mocked(RequestPasswordReset.requestPasswordResetCode).mockResolvedValue({
        success: true,
        error: null,
      });
    ...
    it("should set isSubmitting during API call", async () => {
      ...
      let resolveRequest!: (value: { success: boolean; error: string | null }) => void;
      vi.mocked(RequestPasswordReset.requestPasswordResetCode).mockReturnValue(
        new Promise((resolve) => {
  ...
  describe("submitCodeStep", () => {
    it("should return false and set fieldErrors on validation failure", async () => {
      ...
      expect(ResetUserPassword.resetUserPassword).not.toHaveBeenCalled();
    });
    ...
    it("should advance to success step on success", async () => {
      ...
      vi.mocked(ResetUserPassword.resetUserPassword).mockResolvedValue({
        success: true,
        error: null,
      });
