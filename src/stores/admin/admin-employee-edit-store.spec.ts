...
vi.mock("@/api/admin/admin-employees-api", () => ({
  AdminEmployeesApi: {
...
const { AdminEmployeesApi } = await import("@/api/admin/admin-employees-api");
...
describe("AdminEmployeeEditStore", () => {
  ...
  describe("loadEmployee", () => {
    ...
    it("should handle missing optional fields", async () => {
      ...
      vi.mocked(AdminEmployeesApi.fetchAdminEmployeeById).mockResolvedValue(employee);
      vi.mocked(AdminAppointmentsApi.fetchUnits).mockResolvedValue([]);
      vi.mocked(AdminAppointmentsApi.fetchAdminAppointments).mockResolvedValue({
        data: [],
        total: 0,
    ...
    it("should set error on failure", async () => {
      vi.mocked(AdminEmployeesApi.fetchAdminEmployeeById).mockRejectedValue(new Error("Not found"));
      vi.mocked(AdminAppointmentsApi.fetchUnits).mockResolvedValue([]);
    ...
    it("should still succeed if fetchUnits fails", async () => {
      vi.mocked(AdminEmployeesApi.fetchAdminEmployeeById).mockResolvedValue(mockEmployee);
      vi.mocked(AdminAppointmentsApi.fetchUnits).mockRejectedValue(new Error("fail"));
      vi.mocked(AdminAppointmentsApi.fetchAdminAppointments).mockResolvedValue({
        data: [],
        total: 0,
        page: 1,
  ...
  describe("saveEmployee", () => {
    it("should save employee successfully", async () => {
      ...
      vi.mocked(AdminEmployeesApi.updateAdminEmployee).mockResolvedValue(mockEmployee);

      await useAdminEmployeeEditStore.getState().loadEmployee(1);
      const result = await useAdminEmployeeEditStore.getState().saveEmployee();

      expect(result).toBe(true);
      expect(AdminEmployeesApi.updateAdminEmployee).toHaveBeenCalledWith(
        1,
    ...
    it("should return false when no employee is loaded", async () => {
      ...
      expect(AdminEmployeesApi.updateAdminEmployee).not.toHaveBeenCalled();
    });
    ...
    it("should set saveError on failure", async () => {
      ...
      vi.mocked(AdminEmployeesApi.updateAdminEmployee).mockRejectedValue(new Error("Server error"));
