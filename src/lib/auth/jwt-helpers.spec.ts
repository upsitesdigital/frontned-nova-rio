...
describe("JwtHelpers.decodeJwtPayload", () => {
  ...
  it("should return null for invalid JWT (not 3 parts)", () => {
    expect(JwtHelpers.decodeJwtPayload("not-a-jwt")).toBeNull();
    expect(JwtHelpers.decodeJwtPayload("only.two")).toBeNull();
  });

  it("should return null for malformed base64", () => {
    expect(JwtHelpers.decodeJwtPayload("a.!!!invalid!!!.c")).toBeNull();
  });
