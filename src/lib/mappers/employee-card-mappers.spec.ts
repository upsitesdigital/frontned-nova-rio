...
import { EmployeeCardMappers } from "./employee-card-mappers";

describe("employee-card-mappers", () => {
  describe("EmployeeCardMappers.buildEmployeeContacts", () => {
    it("should include CPF and email for all employees", () => {
      const contacts = EmployeeCardMappers.buildEmployeeContacts(
        "12345678901",
        null,
        "test@example.com",
      );
  ...
  describe("EmployeeCardMappers.buildEmployeeStatus", () => {
    it("should return active status for ACTIVE", () => {
      const status = EmployeeCardMappers.buildEmployeeStatus("ACTIVE");
    ...
    it("should return inactive status for INACTIVE", () => {
      const status = EmployeeCardMappers.buildEmployeeStatus("INACTIVE");
  ...
  describe("EmployeeCardMappers.buildEmployeeDetails", () => {
    it("should show availability range when both values set", () => {
      const details = EmployeeCardMappers.buildEmployeeDetails("08:00", "18:00");
    ...
    it("should show 'Não definida' when from is null", () => {
      const details = EmployeeCardMappers.buildEmployeeDetails(null, "18:00");

      expect(details[0].value).toBe("Não definida");
    });

    it("should show 'Não definida' when to is null", () => {
      const details = EmployeeCardMappers.buildEmployeeDetails("08:00", null);

      expect(details[0].value).toBe("Não definida");
    });

    it("should show 'Não definida' when both are null", () => {
      const details = EmployeeCardMappers.buildEmployeeDetails(null, null);

      expect(details[0].value).toBe("Não definida");
    });
  });

  describe("EmployeeCardMappers.buildEmployeeActions", () => {
    it("should return schedule and edit actions", () => {
      const onSchedule = () => {};
      const onEdit = () => {};

      const actions = EmployeeCardMappers.buildEmployeeActions(onSchedule, onEdit);
