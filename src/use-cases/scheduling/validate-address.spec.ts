...
vi.mock("@/api/client/scheduling-api", () => ({
  SchedulingApi: { fetchCoverageByCep: vi.fn() },
}));
...
describe("validateAddress", () => {
  ...
  describe("success", () => {
    it("should return address when coverage is confirmed", async () => {
      vi.mocked(api.SchedulingApi.fetchCoverageByCep).mockResolvedValue({
        covered: true,
        address: fakeAddress,
        unitId: 1,
        unitName: "Unit 1",
      ...
      const result = await ValidateAddress.validateAddress("20040-020");
  ...
  describe("not covered", () => {
    it("should return error when address is not covered", async () => {
      ...
      const result = await ValidateAddress.validateAddress("99999-999");
  ...
  describe("error handling", () => {
    it("should return error message on API failure", async () => {
      vi.mocked(api.SchedulingApi.fetchCoverageByCep).mockRejectedValue(new Error("Network"));
  ...
  describe("edge cases", () => {
    it("should pass the cep string to the API as-is", async () => {
      ...
      await ValidateAddress.validateAddress("12345-678");

      expect(api.SchedulingApi.fetchCoverageByCep).toHaveBeenCalledWith("12345-678");
    });
  });
});
