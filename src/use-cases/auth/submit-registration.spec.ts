...
const { SubmitRegistration } = await import("./submit-registration");
...
describe("validateRegistrationInput", () => {
  ...
  it("should call validateRegister with name, email, and password", () => {
    ...
    SubmitRegistration.validateRegistrationInput({
      name: "John",
      email: "john@mail.com",
      phone: "11999999999",
      password: "Pass@1234",
  ...
  it("should return errors from validateRegister", () => {
    ...
    const errors = SubmitRegistration.validateRegistrationInput({
      name: "",
      email: "a@b.com",
      phone: "",
      password: "Pass@1234",
...
describe("submitRegistration", () => {
  ...
  it("should return empty errors on successful registration", async () => {
    ...
    const result = await SubmitRegistration.submitRegistration({
      name: "John",
      email: "john@mail.com",
      phone: "11999999999",
      password: "Pass@1234",
  ...
  it("should call registerClient with correct payload including phone", async () => {
    ...
    expect(api.AuthApi.registerClient).toHaveBeenCalledWith({
      name: "John",
      email: "john@mail.com",
      phone: "11999999999",
      password: "Pass@1234",
  ...
  it("should map HttpClientError to field errors via mapApiErrorToField", async () => {
    ...
    vi.mocked(api.AuthApi.registerClient).mockRejectedValue(httpError);
  ...
  it("should return generic email error for non-HttpClientError exceptions", async () => {
    vi.mocked(api.AuthApi.registerClient).mockRejectedValue(new Error("Unknown"));
  ...
  it("should not call mapApiErrorToField for non-HttpClientError", async () => {
    vi.mocked(api.AuthApi.registerClient).mockRejectedValue(new TypeError("type error"));
