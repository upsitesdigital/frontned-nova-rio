...
describe("loadClientCards", () => {
  ...
  it("should return cards on success", async () => {
    ...
    vi.mocked(api.CardsApi.listCards).mockResolvedValue(cards);
  ...
  it("should return empty array when API returns no cards", async () => {
    vi.mocked(api.CardsApi.listCards).mockResolvedValue([]);
  ...
  it("should return error with fallback message on failure", async () => {
    vi.mocked(api.CardsApi.listCards).mockRejectedValue(new Error("Network error"));
  ...
  it("should call listCards exactly once", async () => {
    ...
    await LoadClientCards.loadClientCards();

    expect(api.CardsApi.listCards).toHaveBeenCalledTimes(1);
  });
});
