...
vi.mock("@/api/admin/admin-clients-api", () => ({
  AdminClientsApi: { fetchAdminClients: vi.fn() },
}));
...
describe("loadAdminClients", () => {
  ...
  it("should return mapped clients on success", async () => {
    vi.mocked(api.AdminClientsApi.fetchAdminClients).mockResolvedValue({
      data: [mockApiClient],
      total: 1,
      page: 1,
      limit: 20,
    ...
    const result = await LoadAdminClients.loadAdminClients({ page: 1, limit: 20 });
  ...
  it("should convert status filter to uppercase", async () => {
    ...
    await LoadAdminClients.loadAdminClients({ page: 1, limit: 20, status: "pending" });

    expect(api.AdminClientsApi.fetchAdminClients).toHaveBeenCalledWith(
  ...
  it("should not pass status when filter is 'all'", async () => {
    ...
    await LoadAdminClients.loadAdminClients({ page: 1, limit: 20, status: "all" });

    const params = vi.mocked(api.AdminClientsApi.fetchAdminClients).mock.calls[0][0];
  ...
  it("should pass search when provided", async () => {
    ...
    await LoadAdminClients.loadAdminClients({ page: 1, limit: 20, search: "Fábio" });
  ...
  it("should return error with fallback message on failure", async () => {
    vi.mocked(api.AdminClientsApi.fetchAdminClients).mockRejectedValue(new Error("Network error"));
  ...
  it("should detect auth error on 401", async () => {
    ...
    vi.mocked(api.AdminClientsApi.fetchAdminClients).mockRejectedValue(authError);
