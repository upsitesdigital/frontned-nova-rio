...
vi.mock("@/api/admin/admin-appointments-api", () => ({
  AdminAppointmentsApi: { createAdminAppointment: vi.fn() },
}));
...
vi.mock("@/lib/formatting/date-helpers", () => ({
  DateHelpers: {
...
const { CreateAdminAppointment } = await import("./create-admin-appointment");
...
describe("submitAdminAppointment", () => {
  ...
  describe("validation", () => {
    it("should return validation_error when serviceId is missing", async () => {
      const result = await CreateAdminAppointment.submitAdminAppointment({
        ...validInput,
        serviceId: "",
      });
  ...
  describe("success", () => {
    it("should return success on successful creation", async () => {
      vi.mocked(api.AdminAppointmentsApi.createAdminAppointment).mockResolvedValue({} as never);
    ...
    it("should pass correct payload to API", async () => {
      ...
      await CreateAdminAppointment.submitAdminAppointment(validInput);

      expect(api.AdminAppointmentsApi.createAdminAppointment).toHaveBeenCalledWith({
        date: "2026-03-15",
        startTime: "10:00",
        duration: 60,
    ...
    it("should omit optional fields when empty", async () => {
      ...
      await CreateAdminAppointment.submitAdminAppointment({
        ...validInput,
        employeeId: "",
        locationZip: "",
        notes: "",
      ...
      const payload = vi.mocked(api.AdminAppointmentsApi.createAdminAppointment).mock.calls[0][0];
    ...
    it("should omit employeeId when set to none", async () => {
      ...
      await CreateAdminAppointment.submitAdminAppointment({ ...validInput, employeeId: "none" });
