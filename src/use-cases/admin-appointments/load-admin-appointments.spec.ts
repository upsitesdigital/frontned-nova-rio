...
describe("loadAdminAppointments", () => {
  ...
  it("should return appointments on success", async () => {
    vi.mocked(api.AdminAppointmentsApi.fetchAdminAppointments).mockResolvedValue({
      data: [{ id: 1 }] as never,
      total: 1,
      page: 1,
      limit: 10,
  ...
  it("should pass date param for today viewMode", async () => {
    ...
    await LoadAdminAppointments.loadAdminAppointments({
      page: 1,
      pageSize: 10,
      viewMode: "today",
      statusFilter: "all",
    ...
    const params = vi.mocked(api.AdminAppointmentsApi.fetchAdminAppointments).mock.calls[0][0];
